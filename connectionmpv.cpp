...
Vdv301Connection ConnectionMPV::toVdv301Connection()
{
    ...
    //output.line.lineNameList<<Vdv301InternationalText(this->alias);


    Vdv301DisplayContent displayContent;
    QString language="cs";

    displayContent.displayContentRef="Lcd";
    displayContent.lineInformation.lineNameList<<Vdv301InternationalText(this->alias,language);
    displayContent.destination.destinationNameList<<Vdv301InternationalText(this->smer,language);
    ...
    output.expectedDepartureTime=shiftTimestampByDelay(this->odj,  this->zpoz );
    output.scheduledDepartureTime= this->odj;
    this->ddDoVdv301VehicleMode(dd,output.mainMode,output.subMode);
